...
if currentGreen == 1 or currentGreen == 0:
    nextGreen = [(currentGreen + 1) % noOfSignals,(currentGreen + 2) % noOfSignals]
elif currentGreen == 2 or currentGreen == 3:
    nextGreen = [(currentGreen + 1) % noOfSignals, (currentGreen - 2) % noOfSignals]
...
x = {'right': [0, 0, 0], 'down': [660, 625, 697], 'left': [1400, 1400, 1400], 'up': [693, 727, 757]}
y = {'right': [468,440,498], 'down': [0, 0, 0], 'left': [395, 360, 336], 'up': [800, 800, 800]}
...
signalCoods = [(530, 570), (530, 270),(810, 230), (810, 570)]
...
# we can control the rotate of car in intersection
mid = {'right': {'x': 640, 'y': 485}, 'down': {'x': 715, 'y': 380}, 'left': {'x': 765, 'y': 425},
       'up': {'x': 675, 'y': 470}}
# the rotation type
rotationAngle = 3
...
# We need to generate 4 traffic signals for our simulation.
class TrafficSignal:
...
# This is a class that represents objects of vehicles that we will be generating in the simulation.
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        pygame.sprite.Sprite.__init__(self)
        self.lane = lane
        # self.vehicleClass = vehicleClass Represents the class of the vehicle such as car, bus, truck, or bike
        self.speed = speeds[vehicleClass]  # Represents the speed of the vehicle according to its class
        self.direction_number = direction_number # Represents the direction — 0 for right, 1 for down, 2 for left, and 3 for up.
        self.direction = direction # Represents the direction in text format
        self.x = x[direction][lane] # Represents the current x-coordinate of the vehicle
        self.y = y[direction][lane] # Represents the current y-coordinate of the vehicle
        self.crossed = 0 # Represents whether the vehicle has crossed the signal or not
        self.willTurn = will_turn
        self.turned = 0
        self.rotateAngle = 0
        vehicles[direction][lane].append(self)
        # self.stop = stops[direction][lane]
        self.index = len(vehicles[direction][lane]) - 1 # Represents the relative position of the vehicle among the vehicles moving in the same direction and the same lane
        path = "images/" + direction + "/" + vehicleClass + ".png"
        self.originalImage = pygame.image.load(path) # Represents the image to be rendered
        ...
        if (direction == 'right'):
            if (len(vehicles[direction][lane]) > 1 and vehicles[direction][lane][
                ...
                # if (len(vehicles[direction][lane]) > 1 in the first line of code we check if the car exist before the current car it have
                # need to introduce the stop and the code mean is look vehicles dictionary and then look to direction dictionary and then get
                #  the value of lane
    ...
    def move(self):
        if (self.direction == 'right'):
            if (self.crossed == 0 and self.x + self.currentImage.get_rect().width > stopLines[self.direction]): # if the image has crossed stop line now
                ...
                # The first condition checks if the vehicle has crossed the stop line and updates its 'crossed'
                ...
                """
                This part of the code is responsible for moving the vehicle in the "right" direction. 

                If `self.willTurn` is 1, it means that the vehicle will make a turn at some point. The first condition checks if either:
                - The vehicle has not crossed the stop line yet (`self.crossed == 0`) or 
                - The x-coordinate of the vehicle plus its width is less than the midpoint x-coordinate for this direction (`mid[self.direction]['x']`).

                If any of these conditions are true, it checks if:
                - Either, 
                    - The x-coordinate plus width of the vehicle is less than or equal to its stop coordinate (`self.x + self.currentImage.get_rect().width <= self.stop`) or
                    - Both `currentGreen` and `currentYellow` signals are 0 (green signal) or
                    - The vehicle has already crossed the intersection (`self.crossed == 1`)

                And,
                - Either, 
                    - It's either the first vehicle in that lane OR
                    - The x-coordinate plus width of this vehicles object is less than (x-coordinate minus gap2) of previous vehicle in that lane OR
                    - Previous Vehicle has turned

                If all these conditions are met, then it updates `self.x` by adding `self.speed`.

                Otherwise, if none of those conditions hold true (i.e., else), it checks whether:
                - Either,
                   - X-coordinate plus width of this vehicles object is less than or equal to its stop coordinate OR
                   - This vehicles object has already crossed stop line OR
                   - Both currentGreen and currentYellow signals are 0

                And,
                - Either,
                   - It's either first Vehicle Object in that Lane OR
                   - X-coordinates plus width coordinates for both objects have enough gap between them so they do not collide with each other.

                If all these conditions hold true, it updates `self.x` by adding `self.speed`.
                """
            if (self.willTurn == 1):
                if (self.crossed == 0 or self.x + self.currentImage.get_rect().width < mid[self.direction]['x']):
                    if ((self.x + self.currentImage.get_rect().width <= self.stop or (
                            currentGreen == 0 or currentGreen == 2 and currentYellow == 0) or self.crossed == 1) and (
                    ...
                    if (self.turned == 0):
                        # in this line we specific the rotate type for rotateAngle
                ...
                if ((self.x + self.currentImage.get_rect().width <= self.stop or self.crossed == 1 or (
                        currentGreen == 0 or currentGreen ==2 and currentYellow == 0)) and (
                        self.index == 0 or self.x + self.currentImage.get_rect().width < (
                        vehicles[self.direction][self.lane][self.index - 1].x - gap2) or (
                                vehicles[self.direction][self.lane][self.index - 1].turned == 1))):
            ...
            if (self.willTurn == 1):
                if (self.crossed == 0 or self.y + self.currentImage.get_rect().height < mid[self.direction]['y']):
                    if ((self.y + self.currentImage.get_rect().height <= self.stop or (
                            currentGreen == 1 or currentGreen == 3 and currentYellow == 0) or self.crossed == 1) and (
                ...
                if ((self.y + self.currentImage.get_rect().height <= self.stop or self.crossed == 1 or (
                        currentGreen == 1 or currentGreen == 3 and currentYellow == 0)) and (
                        self.index == 0 or self.y + self.currentImage.get_rect().height < (
                        vehicles[self.direction][self.lane][self.index - 1].y - gap2) or (
                                vehicles[self.direction][self.lane][self.index - 1].turned == 1))):
                    self.y += self.speed
            ...
            if (self.willTurn == 1):
                if (self.crossed == 0 or self.x > mid[self.direction]['x']):
                    if ((self.x >= self.stop or (currentGreen == 2 or currentGreen == 0 and currentYellow == 0) or self.crossed == 1) and (
                ...
                if ((self.x >= self.stop or self.crossed == 1 or (currentGreen == 2 or currentGreen == 0 and currentYellow == 0)) and (
                        self.index == 0 or self.x > (
                        vehicles[self.direction][self.lane][self.index - 1].x + vehicles[self.direction][self.lane][
            ...
            if (self.willTurn == 1):
                if (self.crossed == 0 or self.y > mid[self.direction]['y']):
                    if ((self.y >= self.stop or (currentGreen == 3 or currentGreen == 1 and currentYellow == 0) or self.crossed == 1) and (
                ...
                if ((self.y >= self.stop or self.crossed == 1 or (currentGreen == 3 or currentGreen == 1 and currentYellow == 0)) and (
                        self.index == 0 or self.y > (
                        vehicles[self.direction][self.lane][self.index - 1].y + vehicles[self.direction][self.lane][
